...
Full Image Url: <a href=\"http://www.gamingonlinux.com/uploads/articles/tagline_images/{$article['tagline_image']}\" target=\"_blank\">Click Me</a><br />";
	...
	*/
	...
	$templating->block('comments_top', 'admin_modules/admin_articles_sections/submitted_articles');
	...
	while ($comments = $db->fetch())
	{
		...
		$templating->block('review_comments', 'admin_modules/admin_articles_sections/submitted_articles');
		...
		if (!empty($comments['steam']))
		{
			$steam = "<a href=\"http://steamcommunity.com/id/{$comments['steam']}/\"><img src=\"/templates/default/images/steam.png\" alt=\"steam\" /></a>";
		}

					if (!empty($comments['twitter_on_profile']))
					{
						$twitter = "<a href=\"http://www.twitter.com/{$comments['twitter_on_profile']}/\"><img src=\"/templates/default/images/twitter.gif\" alt=\"twitter\" /></a>";
					}

					if (!empty($comments['website']))
					{
						$website = "<a href=\"{$comments['website']}\"><i class=\"icon-globe\"></i></a>";
					}

					if ($comments['secondary_user_group'] == 6 && $comments['user_group'] != 1 && $comments['user_group'] != 2)
					{
						$donator_badge = '<br />
	...
	$templating->block('bottom', 'admin_modules/admin_articles_sections/submitted_articles');
	...
	<button type="submit" name="act" value="comment" class="btn btn-primary" formaction="' . $config['path'] .'admin.php?module=articles&view=Submitted&aid=' . $_GET['aid'] . '">Post Comment</button>';

	$core->editor('text', $comment, $buttons, $config['path'] .'admin.php?module=articles&view=Submitted&aid=' . $_GET['aid'], null, 'editor_comments');
}
