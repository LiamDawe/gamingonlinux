...
else
{
	...
	{
		$top_image = '<img src="'.core::config('path').'uploads/articles/topimages/'.$article['article_top_image_filename'].'" alt="[articleimage]" class="imgList"><br />';
	}
...
Full Image Url: <a href=\"{$config['path']}uploads/articles/tagline_images/{$article['tagline_image']}\" target=\"_blank\">Click Me</a><br />";
	}
...
if (isset($_POST['act']))
{
	if ($_POST['act'] == 'comment')
	{
		...
		else
		{
			...
			{
				header("Location: {$config['path']}admin.php?module=reviewqueue&aid={$_GET['aid']}&error=doublecomment#editor_comments");
			...
			if (empty($comment))
			{
				header("Location: {$config['path']}admin.php?module=reviewqueue&aid={$_GET['aid']}&error=emptycomment#editor_comments");
			...
			header("Location: {$config['path']}admin.php?module=reviewqueue&aid=$article_id#comments");
