...
{
	$grab_article = $db->sqlquery("SELECT h.`text`, h.`date`, u.`username`, u.`user_id`, a.`title` FROM `article_history` h INNER JOIN `".$dbl->table_prefix."users` u ON h.user_id = u.user_id INNER JOIN `articles` a ON h.article_id = a.article_id WHERE h.id = ?", array($_GET['id']));
