...
if (!isset($_GET['ip_id']))
{
	...
	$db->sqlquery("SELECT a.*, t.title, u.username, u.user_group, u.`avatar`, u.`avatar_gravatar`, u.`gravatar_email`, u.`avatar_uploaded`, u.register_date, u2.username as reported_by_username FROM `articles_comments` a INNER JOIN `articles` t ON a.article_id = t.article_id LEFT JOIN ".$core->db_tables['users']." u ON a.author_id = u.user_id LEFT JOIN ".$core->db_tables['users']." u2 on a.spam_report_by = u2.user_id WHERE a.spam = 1 ORDER BY a.`comment_id` ASC LIMIT ?, 9", array($core->start));
	if ($db->num_rows() > 0)
	{
