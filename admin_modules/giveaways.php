...
else if (isset($_GET['manage']))
{
  ...
  SUM(CASE WHEN game_id = ? THEN 1 ELSE 0 END) AS total
	...
	$db->sqlquery("SELECT k.`game_key`, k.`claimed`, u.`user_id`, u.`username` FROM `game_giveaways_keys` k LEFT JOIN `".$dbl->table_prefix."users` u ON u.`user_id` = k.`claimed_by_id` WHERE k.`game_id` = ? ORDER BY k.`claimed` DESC", array($_GET['manage']));
	while ($keys = $db->fetch())
	{
