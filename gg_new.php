...
//$get_url = $core->htmlentities2stringcode($get_url);
...
foreach ($xml->item as $game)
{
	...
	//$new_title = $core->stringcode2htmlentities($game->title);

	// remove the STUPID characters from game names, seriously developers, fuck off with included them in game names
	//$new_title = preg_replace("/(™|®|©|&trade;|&reg;|&copy;|&#8482;|&#174;|&#169;|&#153;|&#xA9;|&#xAE;|&#x99;)/", "", $game->title);

	// their importer gives us html characters, odd, no one else does
	//$new_title = htmlspecialchars_decode($game->title, ENT_QUOTES);
	...
	if ($game->price != "-" && $game->price > 0)
	{
		...
		$db->sqlquery("SELECT `info`, `provider_id` FROM `game_sales` WHERE `info` = ? AND `provider_id` = ?", array($game->title, 12), 'gamersgate_import.php');
