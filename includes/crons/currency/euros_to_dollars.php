...
$db->sqlquery("SELECT `euros`, `id` FROM `game_sales` WHERE `euros` > 0 and `dollars` = 0");
$get = $db->fetch_all_rows();
foreach ($get as $price)
{
	$dollars = currency('EUR', 'USD', $price['euros']);
	$dollars = round($dollars, 2);
	$db->sqlquery("UPDATE `game_sales` SET `dollars` = ? WHERE `id` = ?", array($dollars, $price['id']));
}

// convert euros_original to dollars_original
$db->sqlquery("SELECT `euros_original`, `id` FROM `game_sales` WHERE `euros_original` > 0 and `dollars_original` = 0");
$get = $db->fetch_all_rows();
foreach ($get as $price)
{
	$dollars = currency('EUR', 'USD', $price['euros_original']);
	$dollars = round($dollars, 2);
