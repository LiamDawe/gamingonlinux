...
$db->sqlquery("SELECT `euros`, `id` FROM `game_sales` WHERE `euros` > 0 and `pounds` = 0");
$get = $db->fetch_all_rows();
foreach ($get as $price)
{
	$pounds = currency('EUR', 'GBP', $price['euros']);
...
$db->sqlquery("SELECT `euros_original`, `id` FROM `game_sales` WHERE `euros_original` > 0 and `pounds_original` = 0");
$get = $db->fetch_all_rows();
foreach ($get as $price)
{
	$pounds = currency('EUR', 'GBP', $price['euros_original']);
	$pounds = round($pounds, 2);
