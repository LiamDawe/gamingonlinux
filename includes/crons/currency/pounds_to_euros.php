...
$db->sqlquery("SELECT `pounds`, `id` FROM `game_sales` WHERE `pounds` > 0 and `euros` = 0");
$get = $db->fetch_all_rows();
foreach ($get as $price)
{
	$euros = currency('GBP', 'EUR', $price['pounds']);
	$euros = round($euros, 2);
	$db->sqlquery("UPDATE `game_sales` SET `euros` = ? WHERE `id` = ?", array($euros, $price['id']));
}

// convert euros_original to dollars_original
$db->sqlquery("SELECT `pounds_original`, `id` FROM `game_sales` WHERE `pounds_original` > 0 and `euros_original` = 0");
$get = $db->fetch_all_rows();
foreach ($get as $price)
{
	$euros = currency('GBP', 'EUR', $price['pounds_original']);
	$euros = round($euros, 2);
