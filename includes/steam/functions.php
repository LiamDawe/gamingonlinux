...
class check_user
{
	...
	function check_that_id($steam_id, $steam_username)
	{
		...
		{
			$db->sqlquery("SELECT `user_id`, `forum_type`, `single_article_page`, `per-page`, `articles-per-page`, `username`, `user_group`, `secondary_user_group`, `banned`, `theme`, `activated`, `in_mod_queue`, `email`, `login_emails` FROM `users` WHERE `steam_id` = ?", array($steam_id));
			$result = $db->fetch();
			if (!empty($result))
			{
				$this->new = 1;
