...
if (!isset($_GET['redo']))
{
	...
	{
		$db->sqlquery("SELECT `user_id` FROM ".$core->db_tables['users']." WHERE `user_id` = ? AND `activation_code` = ?", array($_GET['user_id'], $_GET['code']));
		$count_rows = $db->num_rows();
		if ($count_rows == 1)
		{
			$db->sqlquery("UPDATE ".$core->db_tables['users']." SET `activated` = 1 WHERE `user_id` = ?", array($_GET['user_id']));
...
{
	$db->sqlquery("SELECT `email`, `activated` FROM ".$core->db_tables['users']." WHERE `user_id` = ?", array($_SESSION['user_id']));
	$get_active = $db->fetch();
