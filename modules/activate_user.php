...
else if (isset($_GET['redo']) && $_SESSION['user_id'] != 0)
{
	...
	if ($get_active['activated'] == 0)
	{
		...
		<p>Thanks for registering on <a href=\"{$config['website_url']}{$config['path']}\" target=\"_blank\">{$config['website_url']}{$config['path']}</a>, The best source for linux games and news.</p>
		<p><strong><a href=\"{$config['website_url']}{$config['path']}index.php?module=activate_user&user_id={$_SESSION['user_id']}&code=$code\">You need to activate your account before you can post! Click here to activate!</a></strong></p>
		<p>If you&#39;re new, consider saying hello in the <a href=\"{$config['website_url']}{$config['path']}forum/\" target=\"_blank\">forum</a>.</p>
		<br style=\"clear:both\">
		<div>
		<hr>
		<p>If you haven&#39;t registered at <a href=\"{$config['website_url']}{$config['path']}\" target=\"_blank\">{$config['website_url']}{$config['path']}</a>, Forward this mail to <a href=\"mailto:{$config['contact_email']}\" target=\"_blank\">{$config['contact_email']}</a> with some info about what you want us to do about it.</p>
