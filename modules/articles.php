...
if (isset($_GET['view']) && !isset($_POST['act']))
{
	...
	if ($_GET['view'] == 'multiple')
	{
		...
		if (isset($_GET['catid']))
		{
			...
			$db->sqlquery("SELECT r.article_id, a.author_id, a.title, a.slug, a.tagline, a.text, a.date, a.comment_count, a.guest_username, a.article_top_image, a.article_top_image_filename, a.tagline_image, a.show_in_menu, u.username FROM `article_category_reference` r JOIN `articles` a ON a.article_id = r.article_id LEFT JOIN `users` u on a.author_id = u.user_id WHERE $cat_sql AND a.active = 1 GROUP BY r.article_id $all_sql ORDER BY a.`date` DESC LIMIT {$core->start}, {$_SESSION['articles-per-page']}", $_GET['catid']);
