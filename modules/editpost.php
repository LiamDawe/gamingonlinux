...
if (!isset($_POST['act']))
{
	...
	if (isset($_GET['topic_id']) && is_numeric($_GET['topic_id']))
	{
		...
		{
			$buttons = '<input type="submit" name="act" value="Edit" class="btn btn-primary" />
			<input type="hidden" name="page" value="' . $_GET['page'] . '" />';

			$templating->block('topic');
			...
			$core->editor('text', $topic['topic_text'], $buttons, $config['path'] . 'index.php?module=editpost&amp;topic_id='.$topic['topic_id'].'&reported=' . $reported, 'Topic Title: <input type="text" name="title" value="'.htmlentities($topic['topic_title'], ENT_QUOTES).'" /><br />');
		}
		else
		{
	...
	if (isset($_GET['post_id']) && is_numeric($_GET['post_id']))
	{
		...
		if ($_SESSION['user_id'] == $post['author_id'] || $user->check_group(1,2) == true)
		{
			...
			$buttons = '<input type="submit" name="act" value="Edit" class="btn btn-primary" />
			<input type="hidden" name="page" value="'.$_GET['page'].'" /><input type="hidden" name="topic_id" value="'.$post['topic_id'].'" />';

			$core->editor('text', $post['reply_text'], $buttons, $config['path'] . 'index.php?module=editpost&amp;post_id=' . $post['post_id'] . '&reported={:reported}');
		}
		else
		{
