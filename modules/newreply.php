		...
		else
		{
			...
			else
			{
				...
				foreach ($users_array as $email_user)
				{
					...
					<p><strong>{$_SESSION['username']}</strong> has replied to a forum topic you follow on titled \"<strong><a href=\"{$config['website_url']}{$config['path']}forum/topic/{$topic_id}?page={$postPage}#{$post_id}\">{$title['topic_title']}</a></strong>\". There may be more replies after this one, and you may not get any more emails depending on your email settings in your UserCP.</p>
					<div>
					<hr>
			 		{$email_message}
			 		<hr>
					You can unsubscribe from this topic by <a href=\"{$config['website_url']}{$config['path']}/unsubscribe.php?user_id={$email_user['user_id']}&topic_id={$topic_id}&email={$email_user['email']}\">clicking here</a>, you can manage your subscriptions anytime in your <a href=\"{$config['website_url']}{$config['path']}/usercp.php\">User Control Panel</a>.
					<hr>
			  		<p>If you haven&#39;t registered at <a href=\"{$config['website_url']}{$config['path']}\" target=\"_blank\">{$config['path']}</a>, Forward this mail to <a href=\"mailto:liamdawe@gmail.com\" target=\"_blank\">liamdawe@gmail.com</a> with some info about what you want us to do about it or if you logged in and found no message let us know!</p>
				...
				if ($config['pretty_urls'] == 1)
				{
					...
					header("Location: {$config['path']}index.php?module=viewtopic&topic_id={$topic_id}&page={$postPage}#r{$post_id}");
				}
