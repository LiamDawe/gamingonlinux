    ...
    if ($_POST['act'] == 'Submit')
    {
        ...
        else
        {
            ...
            else if (($parray['submit_article_captcha'] == 1 && $res['success']) || $parray['submit_article_captcha'] == 0)
            {
                ...
                $db->sqlquery("INSERT INTO `articles` SET `author_id` = ?, `guest_username` = ?, `guest_email` = ?, `guest_ip` = ?, `date` = ?, `date_submitted` = ?, `title` = ?, `slug` = ?, `text` = ?, `active` = 0, `submitted_article` = 1, `submitted_unapproved` = 1", array($_SESSION['user_id'], $guest_username, $guest_email, $core->ip, $core->date, $core->date, $title, $title_slug, $text));
