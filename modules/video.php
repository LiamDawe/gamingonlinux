...
$online_list = '';
$db->sqlquery("SELECT `username`, `twitch` FROM `".$dbl->table_prefix."users` WHERE `twitch` != ''");
...
$db->sqlquery("SELECT COUNT(user_id) as count FROM `".$dbl->table_prefix."users` WHERE `twitch` != '' OR `youtube` != ''");
$counter = $db->fetch();
...
$db->sqlquery("SELECT `username`, `youtube`, `twitch` FROM `".$dbl->table_prefix."users` WHERE `twitch` != '' OR `youtube` != '' LIMIT ?, 30", array($core->start));
