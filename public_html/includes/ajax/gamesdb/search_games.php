...
if(isset($_GET['q']) && isset($_GET['return_type']))
{
	...
	{
		$sql = "SELECT DISTINCT c.`name` FROM `sales` s INNER JOIN `calendar` c ON c.id = s.game_id WHERE c.`name` LIKE ? AND `is_emulator` = 0 ORDER BY c.`name` ASC";
	}
	if (isset($_GET['type']) && $_GET['type'] == 'free')
	{
		$sql = "SELECT `id`, `name` FROM `calendar` WHERE `name` LIKE ? AND `free_game` = 1 AND `is_application` = 0 AND `is_emulator` = 0 ORDER BY `name` ASC";		
	}
	if (isset($_GET['type']) && $_GET['type'] == 'games_only')
	{
		$sql = "SELECT `id`, `name` FROM `calendar` WHERE `name` LIKE ? AND `is_game` = 1 ORDER BY `name` ASC ";	
	}
	if (isset($_GET['type']) && $_GET['type'] == 'all_nodlc')
	{
		$sql = "SELECT `id`, `name` FROM `calendar` WHERE `name` LIKE ? AND `is_dlc` = 0 ORDER BY `name` ASC ";	
	}
	if (isset($_GET['type']) && $_GET['type'] == 'dlc_only')
	{
		$sql = "SELECT `id`, `name` FROM `calendar` WHERE `name` LIKE ? AND `is_dlc` = 1 ORDER BY `name` ASC ";	
	}	
