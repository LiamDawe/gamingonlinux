...
class charts
{
	...
	function setup($custom_options = NULL)
	{
		...
		$this->chart_options['bundle_outside_top10'] = 0;
	...
	function get_labels($chart_data, $type = 'normal')
	{
		...
		$bundled = array();
		if ($this->chart_options['bundle_outside_top10'] == 1)
		{
			// make an array of everything after the top 10 to bundle them
			$remainers = array_slice($this->labels_raw_data, 10);
			$total_data = 0;

			foreach ($remainers as $to_bundle)
			{
				$total_data = $total_data + $to_bundle['data'];
			}

			$bundled[0]['label_id'] = 'bundled';
			$bundled[0]['name'] = 'Others';
			$bundled[0]['data'] = $total_data;

			if ($bundled[0]['data'] > 0)
			{
				$get_labels = array_merge($get_labels, $bundled);
			}
		}
