...
{
    $find_user = $dbl->run("SELECT `user_id` FROM `users` WHERE `profile_address` = ?", array(core::$url_command[1]))->fetch();
    if ($find_user)
    {
            ...
            else
            {
                ...
                else if (($profile['banned'] == 1 && $user->check_group([1,2,5]) == true) || $profile['banned'] == 0)
                {
                    ...
                    if ($profile['pc_info_public'] == 1)
                    {
                        ...
                        {
                            $profile_link = '/profiles/' . $profile['profile_address'];
                        }
                        else
                        {
                            $profile_link = "/profiles/{$profile['user_id']}";
                        }
                    ...
                    if ($forum_ids)
                    {
                        ...
                        $posts_execute = $dbl->run($posts_sql, array_merge($forum_ids,[$profile['user_id']]))->fetch_all();
