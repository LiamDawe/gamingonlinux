	...
	else if (isset($_POST['register']))
	{
		...
		if ($core->config('captcha_disabled') == 1 || ($core->config('captcha_disabled') == 0 && ($core->config('register_captcha') == 1 && $res['success']) || $core->config('register_captcha') == 0))
		{
			...
			if (time() > $register_time)
			{
				...
				if ($_POST['register'] == 'Register')
				{
					$do_register = $dbl->run("INSERT INTO `users` SET `username` = ?, `password` = ?, `email` = ?, `ip` = ?, `register_date` = ?, `last_login` = ?, `activation_code` = ?, `timezone` = ?", array($_POST['username'], $safe_password, $email, core::$ip, core::$date, core::$date, $code, $timezone));
				}

				if ($_POST['register'] == 'twitter')
				{
					$do_register = $dbl->run("INSERT INTO `users` SET `username` = ?, `email` = ?, `ip` = ?, `register_date` = ?, `last_login` = ?, `oauth_provider` = ?, `oauth_uid` = ?, `twitter_username` = ?, `activation_code` = ?, `timezone` = ?", array($_POST['username'], $email, core::$ip, core::$date, core::$date, $_SESSION['twitter_data']['oauth_provider'], $_SESSION['twitter_data']['uid'], $_SESSION['twitter_data']['twitter_username'], $code, $timezone));
				}

				if ($_POST['register'] == 'steam')
				{
					$do_register = $dbl->run("INSERT INTO `users` SET `username` = ?, `email` = ?, `ip` = ?, `register_date` = ?, `last_login` = ?, `steam_id` = ?, `steam_username` = ?, `activation_code` = ?, `timezone` = ?", array($_POST['username'], $email, core::$ip, core::$date, core::$date, $_SESSION['steam_id'], $_SESSION['steam_username'], $code, $timezone));
				}
					
				if ($_POST['register'] == 'google')
				{
					$do_register = $dbl->run("INSERT INTO `users` SET `username` = ?, `email` = ?, `ip` = ?, `register_date` = ?, `last_login` = ?, `google_email` = ?, `activation_code` = ?, `timezone` = ?", array($_POST['username'], $email, core::$ip, core::$date, core::$date, $_SESSION['google_data']['google_email'], $code, $timezone));
				}
