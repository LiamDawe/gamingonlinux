...
<atom:link href="http://www.gamingonlinux.com/podcast_rss_ogg.php" rel="self" type="application/rss+xml"/>
...
$articles = $dbl->run("SELECT a.* FROM `articles` a LEFT JOIN `article_category_reference` c ON c.article_id = a.article_id WHERE a.`active` = 1 AND c.`category_id` = 97 ORDER BY a.`date` DESC LIMIT 30")->fetch_all();
...
foreach ($articles as $line)
{
	...
	if (isset($matches[1]))
	{
		...
		<enclosure url=\"$item_url\" type=\"audio/ogg\" length=\"$size\" />
