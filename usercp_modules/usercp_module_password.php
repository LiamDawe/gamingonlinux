...
if (isset($_POST['act']))
{
	if ($_POST['act'] == 'request')
	{
		...
		$dbl->run("UPDATE `users` SET `password` = ? WHERE `user_id` = ?", array($safe_password, $_SESSION['user_id']));
	...
	if ($_POST['act'] == 'Update')
	{
		...
		// find current password
		$grab_current_password = $dbl->run("SELECT `username`, `password`, `steam_id`, `oauth_uid`, `email` FROM `users` WHERE `user_id` = ?", array($_SESSION['user_id']))->fetch();
		...
		$dbl->run("UPDATE `users` SET `password` = ? WHERE `user_id` = ?", array($new_password_safe, $_SESSION['user_id']));
