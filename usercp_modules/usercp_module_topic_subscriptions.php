...
if (!isset($_GET['go']))
{
	...
	$total_pages = $dbl->run("SELECT COUNT(t.`topic_id`) FROM `forum_topics` t INNER JOIN `forum_topics_subscriptions` s ON t.topic_id = s.topic_id WHERE s.`user_id` = ?", array($_SESSION['user_id']))->fetchOne();
	...
	$post_res = $dbl->run("SELECT t.*, s.user_id, u.`username`, u.avatar, u.avatar_uploaded, u.avatar_gallery, u2.`username` as `username_last`, u2.`user_id` as `user_id_last` FROM `forum_topics` t INNER JOIN `forum_topics_subscriptions` s ON t.topic_id = s.topic_id INNER JOIN `users` u ON t.`author_id` = u.`user_id` LEFT JOIN `users` u2 ON t.`last_post_user_id` = u2.`user_id` WHERE s.`user_id`= ? ORDER BY t.`last_post_date` DESC LIMIT ?, 9", array($_SESSION['user_id'], $core->start))->fetch_all();
